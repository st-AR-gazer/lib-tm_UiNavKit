namespace UiNavKit {
    namespace Inspectors {
        namespace ControlTree {
            ...
            bool _ControlTreeSelectionCopyValueText(
            ...
            void _ControlTreeSnapshotLinesRec(
                CControlBase@ n,
                ...
                string stack = _ControlTreeShortText(UiNav::CleanUiFormatting(n.StackText), 140);
                string text = _ControlTreeShortText(UiNav::CleanUiFormatting(UiNavKit::Runtime::ReadText(n)), 160);
                bool vis = UiNavKit::Runtime::IsEffectivelyVisible(n);
                uint childCount = UiNavKit::Runtime::_ChildrenLen(n);

                string line = ind + relPath + " : " + UiNavKit::Runtime::NodeTypeName(n)
                    + " vis=" + (vis ? "true" : "false")
                    + " children=" + childCount;
            ...
            void _ControlTreeSetVisibleSubtree(CControlBase@ node, bool visible, int depth = 0, int maxDepth = 128) {
                ...
                for (uint i = 0; i < len; ++i) {
                    auto ch = UiNavKit::Runtime::_ChildAt(node, i);
                    if (ch is null) continue;
            ...
            bool _BuildControlTreeSelectionContext(ControlTreeSelectionContext@&out ctx, string &out err) {
                ...
                @built.sel = sel;
                built.text = UiNav::CleanUiFormatting(UiNavKit::Runtime::ReadText(sel));
                if (built.text.Length > 200) built.text = built.text.SubStr(0, 200) + "...";
                built.selIdName = sel.IdName.Trim();
                built.selStackText = _ControlTreeShortText(UiNav::CleanUiFormatting(sel.StackText), 220);
                built.childCount = UiNavKit::Runtime::_ChildrenLen(sel);
                built.isVisible = UiNavKit::Runtime::IsEffectivelyVisible(sel);
            ...
            void _RenderControlTreeSelectionHeader(ControlTreeSelectionContext@ ctx) {
                ...
                string title = UiNavKit::Runtime::NodeTypeName(ctx.sel);
                ...
                if (ctx.selector.Length > 0) {
                    ...
                    UI::SameLine();
                    _ControlTreeSelectionCopyValueText(
                        ctx.selector,
                        ctx.selector,
                        "controlTree-summary-selector",
                        true
                    );
                } else {
                    UI::TextDisabled("Selector:");
                ...
                _ControlTreeSelectionCopyValueText(
                    dispLine,
                    (ctx.dispPath.Length > 0 ? ctx.dispPath : "<empty>"),
                    "controlTree-summary-display"
                );
                UI::EndChild();
            }
            ...
            void _RenderControlTreeSelection() {
                ...
                if (!_BuildControlTreeSelectionContext(ctx, selErr) || ctx is null) {
                    UiNavKit::Diagnostics::_DiagBreadcrumb(
                        "ControlTree selection: resolve failed: " + selErr,
