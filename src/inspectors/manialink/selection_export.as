        ...
        namespace ManiaLink {
            ...
            void _RenderMlSelectionExport(MlSelectionContext@ ctx) {
                ...
                if (UI::CollapsingHeader("Snapshot")) {
                    ...
                    S_MlSnapshotIncludeChildren = UI::Checkbox(
                        "Include children in snapshot",
                        S_MlSnapshotIncludeChildren
                    );
                    ...
                    if (_MlActionText("Selected snapshot -> clipboard", "ml-snapshot-copy-selected")) {
                        bool ok = UiNav::ML::CopySnapshotToClipboard(
                            ctx.sel,
                            S_MlSnapshotIncludeChildren,
                            S_MlSnapshotMaxDepth
                        );
                    ...
                    if (UI::Button("Save selected snapshot")) {
                        bool ok = UiNav::ML::SaveSnapshotToFile(
                            ctx.sel,
                            S_MlSnapshotPath,
                            S_MlSnapshotIncludeChildren,
                            S_MlSnapshotMaxDepth
                    ...
                    g_MlSnapshotApplySelector = UI::InputText(
                        "Apply selector (current layer root)",
                        g_MlSnapshotApplySelector
                    );
                ...
                if (UI::CollapsingHeader("Style Pack")) {
                    ...
                    UI::SameLine();
                    S_MlStylePackIncludeTextValues = UI::Checkbox(
                        "Include text values",
                        S_MlStylePackIncludeTextValues
                    );
                    ...
                    if (UI::Button("Load pack file")) {
                        ...
                        ("Loaded style pack entries: " + g_MlStylePackEntries.Length + ".") : "Failed to load style pack file.";
                    }

                    UI::Separator();
                    S_MlStylePackApplyChildren = UI::Checkbox(
                        "Apply children when applying pack",
                        S_MlStylePackApplyChildren
                    );
                ...
                if (UI::CollapsingHeader("Dump")) {
                    ...
                    if (UI::Button("Dump layer subtree")) {
                        ...
                        UiNavKit::Diagnostics::DumpMlSubtreeToFile(root, "LAYER", S_MlDumpOnlyOpenPaths, uiPath);
                    }
                    UI::SameLine();
                    ...
                    if (UI::Button("Dump layer page XML")) {
                        ...
                        string appPrefix = _MlAppPrefixByKind(g_SelectedMlAppKind);
                        UiNavKit::Diagnostics::DumpMlLayerPageToFile(layer, g_SelectedMlLayerIx, appPrefix);
                    }
                    UI::SameLine();
                    if (UI::Button("Dump ALL layer pages (slow)")) {
                        string appPrefix = _MlAppPrefixByKind(g_SelectedMlAppKind);
                        UiNavKit::Diagnostics::DumpAllMlLayerPagesToFolder(g_SelectedMlAppKind, appPrefix);
                    }
                    UI::SameLine();
