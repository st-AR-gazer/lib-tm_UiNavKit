    ...
    namespace Inspectors {
        namespace ManiaLink {
            ...
            int _MlPreferredSourceKind(bool hasPlayground, bool hasMenu, bool hasEditor) {
                ...
                auto current = UiNavKit::Runtime::GetManiaApp();
                if (current !is null) {
                    auto menu = UiNavKit::Runtime::GetManiaAppMenu();
                    if (hasMenu && menu !is null && current is menu) return 1;

                    auto playground = UiNavKit::Runtime::GetManiaAppPlayground();
            ...
            void _RenderMlTreePane(int appKind) {
                ...
                if (UI::IsItemHovered()) {
                    UI::SetTooltip(hasSelection ? "Copy the live geometry/bounds data for the current ManiaLink selection." : "Select a ManiaLink node first.");
                }
                if (g_MlViewLayerIndex < -1) g_MlViewLayerIndex = -1;

                if (!_MlSourceAvailable(appKind)) {
                    UiNavKit::Diagnostics::_DiagBreadcrumb(
                        "ML pane: source unavailable (return)",
                        "_RenderMlTreePane",
                        true
                    );
                ...
                for (uint i = 0; i < g_MlLayerFavorites.Length; ++i) {
                    ...
                    if (UI::Selectable(label + "##ml-favorite-" + i, selected)) {
                        ...
                        if (targetIx >= 0) {
                            ...
                            UI::ShowNotification(
                                "UiNavKit",
                                "Favorite layer not found in " + _MlAppNameByKind(appKind) + ".",
                                6000
                            );
                        }
                    }
            ...
            void _RenderMlLayerTree(
                CGameUILayer@ layer,
                int layerIx,
                const string &in layerUiPath,
                const string &in filter,
                const array<_SearchTerm@> &in searchTerms
            ) {
                if (g_MlRowsTruncated) return;
                ...
                if (UI::BeginPopup(layerPopupId)) {
                    ...
                    if (hasLayerLocks) {
                        ...
                        if (UI::MenuItem(toggleLabel)) {
                            int changed = _MlSetLayerValueLocksEnabled(
                                g_MlActiveAppKind,
                                layerIx,
                                !anyLayerLocksEnabled
                            );
                            if (changed > 0) {
                        ...
                        if (UI::MenuItem("Clear value locks (layer)")) {
                            ...
                            ("Removed " + removed + " layer value lock(s).") : "No layer value locks to clear.";
                        }
                    }
                    UI::EndPopup();
                }
