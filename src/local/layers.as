...
namespace Layers {
    ...
    ManiaLinkSource _SourceFromApp(CGameManiaApp@ app) {
        auto menu = GetManiaAppMenu();
        if (menu !is null && app is menu) return ManiaLinkSource::Menu;
        auto pg = GetManiaAppPlayground();
        if (pg !is null && app is pg) return ManiaLinkSource::Playground;
        return ManiaLinkSource::CurrentApp;
    }

    CGameUILayer@ Ensure(const string &in key, const string &in page, bool visible = true, bool persistAcrossReloads = false) {
        return UiNav::ML::Layers::EnsureOwned(key, page, ManiaLinkSource::CurrentApp, visible, persistAcrossReloads);
    }

    CGameUILayer@ EnsureAtApp(const string &in key, const string &in page, CGameManiaApp@ app,
        bool visible = true, bool persistAcrossReloads = false)
    {
