namespace UiNavKit {
    namespace Diagnostics {
        ...
        void _PushIntegrationFixture(
            const string &in id,
            const string &in title,
            IntegrationFixtureCategory category,
            IntegrationFixtureOutcome outcome,
        ...
        string _FixtureOpSummary(const UiNav::OpResult@ res) {
        ...
        UiNav::Target@ _NewCtFixtureTarget(
        ...
        UiNav::Target@ _NewMlFixtureTarget(
            const string &in name,
            UiNav::ManiaLinkSource source,
            const string &in pageNeedle,
            const string &in rootControlId,
            const string &in selector
        ) {
        ...
        bool _FixtureOwnedMlWriteRoundtrip(string &out detail, bool &out skipped) {
            ...
            auto layer = UiNav::ML::Layers::EnsureOwned(key, page, UiNav::ManiaLinkSource::CurrentApp, true);
            if (layer is null) {
                detail = "EnsureOwned returned null for the self-contained owned layer.";
                return false;
            }

            auto target = _NewMlFixtureTarget(
                "Integration fixture owned entry",
        ...
        bool _FixtureAutoManiaIconChooser(string &out detail, bool &out skipped) {
            ...
            string titleCmp = UiNavKit::Runtime::NormalizeForCompare(title).ToLower();
        ...
        void _RunIntegrationFixturesTask() {
            ...
            try {
                ...
                if (wasSkipped) {
                    _PushIntegrationFixture(
                        "IF-UINAV-001",
                        "Owned ML write roundtrip",
                        IntegrationFixtureCategory::SelfContainedWrite,
                        IntegrationFixtureOutcome::Skip,
                        detail
                    );
                    skipped++;
                } else if (ok) {
                    _PushIntegrationFixture(
                        ...
                        "Owned ML write roundtrip",
                        IntegrationFixtureCategory::SelfContainedWrite,
                        IntegrationFixtureOutcome::Pass,
                        detail
                    );
                    passed++;
                } else {
                    _PushIntegrationFixture(
                        ...
                        "Owned ML write roundtrip",
                        IntegrationFixtureCategory::SelfContainedWrite,
                        IntegrationFixtureOutcome::Fail,
                        detail
                    );
                    failed++;
                }
                ...
                if (wasSkipped) {
                    _PushIntegrationFixture(
                        "IF-CONS-001",
                        "PlayerSearch filter flow",
                        IntegrationFixtureCategory::ConsumerFlow,
                        IntegrationFixtureOutcome::Skip,
                        detail
                    );
                    skipped++;
                ...
                if (wasSkipped) {
                    _PushIntegrationFixture(
                        "IF-CT-001",
                        "AutoMania close item editor",
                        IntegrationFixtureCategory::ControlTreeRead,
                        IntegrationFixtureOutcome::Skip,
                        detail
                    );
                    skipped++;
                ...
                _PushIntegrationFixture(
                    "IF-UINAV-ERR",
                    "Integration fixtures runner",
                    IntegrationFixtureCategory::ConsumerFlow,
                    IntegrationFixtureOutcome::Fail,
                    "Unhandled exception while executing integration fixtures."
                );
                failed++;
            }
        ...
        void RenderIntegrationFixturesUI() {
            ...
            UI::Text(releaseCoverageOk ? "\\$9fdRelease behavior coverage satisfied for this run.\\$z" : "\\$fd8Release behavior coverage is incomplete for this run.\\$z");
