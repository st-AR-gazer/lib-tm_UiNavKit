...
namespace Builder {
    ...
    string _SelectorColorizeType(const string &in typeName) {
        ...
        string c = "\\$ddd";
        if (low.Contains("frame")) c = "\\$9fd";
        else if (low.Contains("label") || low.Contains("text")) c = "\\$bff";
        else if (low.Contains("quad") || low.Contains("sprite") || low.Contains("image")) c = "\\$fcb";
        else if (low.Contains("entry") || low.Contains("input")) c = "\\$fd8";
        else if (low.Contains("gauge") || low.Contains("meter") || low.Contains("progress")) c = "\\$fc8";
        return c + typeName + "\\$z";
    }
    ...
    void _RenderSelectorToolbar() {
        ...
        // Source combo + Include hidden on same line
        UI::SameLine();
    ...
    void _RenderSelectorHitList() {
        ...
        // Filter bar: filter input + hit count + Copy All + Last pick
        ...
        // Column width pre-pass (filtered only)
        uint rankChars = 1;
        uint typeChars = 4;
        uint idChars = 4;
        uint pathChars = 6;
        ...
        if (UI::BeginTable("##bv-selector-hits-table", 7, tblFlags)) {
            ...
            for (uint i = 0; i < g_SelectorHits.Length; ++i) {
                ...
                if (h.textPreview.Length > 0 && UI::IsItemHovered())
                    UI::SetTooltip(h.textPreview);
                UI::TableSetColumnIndex(3);
                ...
                // Row hover tooltip with App/Layer info
                if (UI::IsItemHovered()) {
                    UI::SetTooltip(_AppKindLabel(h.appKind) + " L" + h.layerIx
                        + "  |  z=" + h.zIndex + "  depth=" + h.depth);
                }
            }
    ...
    void _RenderSelectorHitInspector(SelectorHitRow@ h, int rank) {
        ...
        if (UI::Button(Icons::ShareSquareO + " ML##bv-selector-sync-ml-one")) {
            ...
            g_SelectorStatus = ok
                ? "Synced to ManiaLink UI."
                : "Could not sync to ManiaLink UI.";
        }
        UI::SameLine();
        if (UI::Button(Icons::ShareSquareO + " CT##bv-selector-sync-ct-one")) {
            ...
            g_SelectorStatus = ok
                ? "Synced to ControlTree."
                : "Could not sync to ControlTree.";
        }
        UI::SameLine();
        ...
        // Overview tab: compact horizontal summary of the most important fields
        if (UI::BeginTabItem(Icons::ThList + " Overview##bv-sel-ov")) {
            if (UI::BeginTable("##bv-sel-ov-tbl", 4, UI::TableFlags::SizingStretchProp)) {
                ...
                UI::TableNextRow();
                UI::TableSetColumnIndex(0); UI::TextDisabled("Type");
                UI::TableSetColumnIndex(1); UI::Text(h.typeName);
                UI::TableSetColumnIndex(2); UI::TextDisabled("ID");
                UI::TableSetColumnIndex(3); UI::Text(h.controlId.Length > 0 ? h.controlId : "\\$888<none>\\$z");

                UI::TableNextRow();
                UI::TableSetColumnIndex(0); UI::TextDisabled("Path");
                UI::TableSetColumnIndex(1); UI::Text("/" + (h.path.Length > 0 ? h.path : "<root>"));
                UI::TableSetColumnIndex(2); UI::TextDisabled("App");
                UI::TableSetColumnIndex(3); UI::Text(_AppKindLabel(h.appKind) + " L" + h.layerIx);

                UI::TableNextRow();
                UI::TableSetColumnIndex(0); UI::TextDisabled("Pos");
                UI::TableSetColumnIndex(1); UI::Text(_FmtVec2(h.absPos));
                UI::TableSetColumnIndex(2); UI::TextDisabled("Size");
                UI::TableSetColumnIndex(3); UI::Text(_FmtVec2(h.absSize));

                UI::TableNextRow();
                UI::TableSetColumnIndex(0); UI::TextDisabled("Vis");
                UI::TableSetColumnIndex(1); UI::Text(h.visibleEffective ? "\\$9fdYes\\$z" : "\\$f88No\\$z");
                UI::TableSetColumnIndex(2); UI::TextDisabled("Z");
                UI::TableSetColumnIndex(3); UI::Text(tostring(h.zIndex));

                if (h.classList.Length > 0) {
                    UI::TableNextRow();
                    UI::TableSetColumnIndex(0); UI::TextDisabled("Class");
                    UI::TableSetColumnIndex(1); UI::Text(h.classList);
                }
                if (h.textPreview.Length > 0) {
                    UI::TableNextRow();
                    UI::TableSetColumnIndex(0); UI::TextDisabled("Text");
                    UI::TableSetColumnIndex(1); UI::Text(h.textPreview);
                }
        ...
        if (UI::BeginTabItem(Icons::Eye + " Visibility##bv-sel-vis")) {
            if (UI::BeginTable("##bv-sel-vis-tbl", 2, UI::TableFlags::SizingStretchProp)) {
                ...
                _RenderSelectorLabelValue("Ancestor", h.hiddenByAncestor ? "\\$f88Hidden\\$z" : "\\$9fdVisible\\$z");
        ...
        if (UI::BeginTabItem(Icons::Clone + " Layer##bv-sel-layer")) {
            if (UI::BeginTable("##bv-sel-layer-tbl", 2, UI::TableFlags::SizingStretchProp)) {
                ...
                _RenderSelectorLabelValue("Attach ID", h.layerAttachId.Length > 0 ? h.layerAttachId : "\\$888<none>\\$z");
                _RenderSelectorLabelValue("ManiaLink", h.manialinkName.Length > 0 ? h.manialinkName : "\\$888<none>\\$z");
                UI::EndTable();
            }
            UI::EndTabItem();
        }
        ...
        if (UI::BeginTabItem(Icons::Cog + " Advanced##bv-sel-adv")) {
            ...
            S_SelectorSyncMlSelection = UI::Checkbox("Sync ML on pick##bv-selector-sync", S_SelectorSyncMlSelection);
            S_SelectorSyncControlTreeSelection = UI::Checkbox("Sync CT on pick##bv-selector-sync-ct", S_SelectorSyncControlTreeSelection);
            UI::EndTabItem();
        }
