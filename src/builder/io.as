namespace UiNavKit {
    namespace Builder {
        ...
        void _AddDiag(
            ...
            const string &in nodeUid = ""
        ) {
            if (doc is null) return;
            auto d = UiNav::Builder::BuilderDiagnostic();
            d.code = code;
            d.severity = severity;
            d.message = message;
            d.nodeUid = nodeUid;
        ...
        void _MapKnownAttrs(UiNav::Builder::BuilderNode@ node, const dictionary &in attrs) {
            ...
            for (uint i = 0; i < keys.Length; ++i) {
                ...
                if (!consumed && node.kind == "quad") {
                    if (lk == "image") {
                        node.typed.image = v;
                        consumed = true;
                    } else if (lk == "imagefocus") {
                        node.typed.imageFocus = v;
                        consumed = true;
                    } else if (lk == "alphamask") {
                        node.typed.alphaMask = v;
                        consumed = true;
                    } else if (lk == "style") {
                        node.typed.style = v;
                        consumed = true;
                    } else if (lk == "substyle") {
                        node.typed.subStyle = v;
                        consumed = true;
                    } else if (lk == "bgcolor") {
                        node.typed.bgColor = v;
                        consumed = true;
                    } else if (lk == "bgcolorfocus") {
                        node.typed.bgColorFocus = v;
                        consumed = true;
                    } else if (lk == "modulatecolor") {
                        node.typed.modulateColor = v;
                        consumed = true;
                    } else if (lk == "colorize") {
                        node.typed.colorize = v;
                        consumed = true;
                    } else if (lk == "opacity") {
                        node.typed.opacity = _ParseFloat(v, node.typed.opacity);
                        consumed = true;
                    } else if (lk == "keepratio") {
                        node.typed.keepRatioMode = _ParseInt(v, node.typed.keepRatioMode);
                        consumed = true;
                    } else if (lk == "blend") {
                        node.typed.blendMode = _ParseInt(v, node.typed.blendMode);
                        consumed = true;
                    }
                }
                ...
                if (!consumed && node.kind == "label") {
                    if (lk == "text") {
                        ...
                        consumed = true;
                    } else if (lk == "textprefix") {
                        node.typed.textPrefix = v;
                        consumed = true;
                    } else if (lk == "textcolor") {
                        node.typed.textColor = v;
                        consumed = true;
                    } else if (lk == "opacity") {
                        node.typed.opacity = _ParseFloat(v, node.typed.opacity);
                        consumed = true;
                    } else if (lk == "maxline") {
                        node.typed.maxLine = _ParseInt(v, node.typed.maxLine);
                        consumed = true;
                    } else if (lk == "autonewline") {
                        node.typed.autoNewLine = _ParseBool(v, node.typed.autoNewLine);
                        consumed = true;
                    } else if (lk == "linespacing") {
                        node.typed.lineSpacing = _ParseFloat(v, node.typed.lineSpacing);
                        consumed = true;
                    } else if (lk == "italicslope") {
                        node.typed.italicSlope = _ParseFloat(v, node.typed.italicSlope);
                        consumed = true;
                    } else if (lk == "appendellipsis") {
                        node.typed.appendEllipsis = _ParseBool(v, node.typed.appendEllipsis);
                        consumed = true;
                    } else if (lk == "style") {
                        node.typed.style = v;
                        consumed = true;
                ...
                if (!consumed && (node.kind == "entry" || node.kind == "textedit")) {
                    if (lk == "default" || lk == "value" || lk == "text") {
                        node.typed.value = v;
                        consumed = true;
                    } else if (lk == "textformat") {
                        node.typed.textFormat = _ParseInt(v, node.typed.textFormat);
                        consumed = true;
                    } else if (lk == "textsize") {
                        node.typed.textSize = _ParseFloat(v, node.typed.textSize);
                        consumed = true;
                        ...
                        node.typed.opacity = _ParseFloat(v, node.typed.opacity);
                        consumed = true;
                    } else if (lk == "maxlen" || lk == "maxlength") {
                        node.typed.maxLength = _ParseInt(v, node.typed.maxLength);
                        consumed = true;
                    } else if (lk == "maxline") {
                        node.typed.maxLine = _ParseInt(v, node.typed.maxLine);
                        consumed = true;
        ...
        int _AppendImportedNode(
            UiNav::Builder::BuilderDocument@ doc,
            const string &in tagNameRaw,
            const dictionary &in attrs,
            int parentIx,
            int start,
            int end
        ) {
        ...
        UiNav::Builder::BuilderDocument@ ImportFromXml(
            const string &in xmlText,
            const string &in sourceKind = "import_xml",
            const string &in sourceLabel = ""
        ) {
            return UiNav::Builder::ImportXml(xmlText, sourceKind, sourceLabel);
        }

        void _EmitAttr(
            array<string> &inout attrs,
            const string &in key,
            const string &in val,
        ...
        void _EmitNode(const UiNav::Builder::BuilderDocument@ doc, int nodeIx, int depth, array<string> &inout lines) {
            if (doc is null) return;
            ...
            if (node.typed !is null) {
                ...
                if (node.kind == "frame") {
                    ...
                    if (node.typed.keepRatioMode != 0) _EmitAttr(
                        attrs,
                        "keepratio",
                        "" + node.typed.keepRatioMode,
                        false
                    );
                    ...
                    if (node.typed.lineSpacing != 0.0f) _EmitAttr(
                        attrs,
                        "linespacing",
                        "" + node.typed.lineSpacing,
                        false
                    );
                    if (node.typed.italicSlope != 0.0f) _EmitAttr(
                        attrs,
                        "italicslope",
                        "" + node.typed.italicSlope,
                        false
                    );
        ...
        string ExportToXml(const UiNav::Builder::BuilderDocument@ doc) {
            return UiNav::Builder::ExportXml(doc);
