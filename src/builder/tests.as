namespace UiNavKit {
    namespace Builder {
        ...
        int _FindFirstNodeByTag(const UiNav::Builder::BuilderDocument@ doc, const string &in tagLower) {
        ...
        void RunAcceptanceSelfTests() {
            ...
            int failed = 0;
            {
                ...
                _PushTest(
                    "AT-M1-001",
                    ok,
                    ok ? "Known controls imported/exported." : "Expected frame/quad/label/entry in output."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest("AT-M1-002", ok, ok ? "Unknown attrs preserved." : "Unknown attr was not preserved.");
                if (ok) {
                    passed++;
                } else {
                    failed++;
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-003",
                    ok,
                    ok ? "Script/stylesheet blocks preserved." : "Script/stylesheet data missing after roundtrip."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-004",
                    ok,
                    ok ? "Unknown tag subtree preserved." : "Unknown tag subtree was not preserved."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-004B",
                    ok,
                    ok ? "Implicit frame clipping preserved without synthetic zero clip bounds." : "Implicit frame clipping roundtrip synthesized clip bounds."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-004C",
                    ok,
                    ok ? "Explicit frame clip bounds preserved." : "Explicit frame clip bounds were not preserved."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest("AT-M1-005", ok, ok ? "Operation chain undo/redo works." : "Undo/redo chain failed.");
                if (ok) {
                    passed++;
                } else {
                    failed++;
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-006",
                    ok,
                    ok ? "Reparenting allows containers and rejects leaf/cycle targets." : "Reparenting failed container move or accepted an invalid target."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-007",
                    ok,
                    ok ? "Structure actions move nodes to parent/root and reorder sibling/root order." : "Structure actions failed to move or reorder nodes correctly."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                if (label !is null && label.typed !is null) {
                    ...
                    vec2 clampedMax = ClampBuilderNodeLocalPosToScreen(
                        g_Doc,
                        labelIx,
                        vec2(999.0f, 999.0f),
                        vec2(160.0f, 90.0f)
                    );
                    vec2 clampedMin = ClampBuilderNodeLocalPosToScreen(
                        g_Doc,
                        labelIx,
                        vec2(-999.0f, -999.0f),
                        vec2(160.0f, 90.0f)
                    );
                    vec2 unchanged = ClampBuilderNodeLocalPosToScreen(
                        g_Doc,
                        labelIx,
                        vec2(10.0f, 5.0f),
                        vec2(160.0f, 90.0f)
                ...
                _PushTest(
                    "AT-M1-008",
                    ok,
                    ok ? "Position slider clamp keeps nodes on-screen." : "Position slider clamp did not produce expected on-screen bounds."
                );
                if (ok) {
                    passed++;
                } else {
            ...
            }
            {
                ...
                _PushTest(
                    "AT-M1-009",
                    ok,
                    ok ? "Sticky snap locks to screen guides and other builder nodes." : "Sticky snap failed against screen or builder-node guides."
                );
                if (ok) {
                    passed++;
                } else {
