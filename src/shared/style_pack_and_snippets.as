namespace UiNavKit {
    ...
    void _BuildMlChains(string &out rootId, string &out idChain, string &out mixedChain, string &out idList) {
        ...
        if (rootId.Length > 0) {
            if (idChain.Length > 0) {
                idChain = "#" + rootId + "/" + idChain;
            } else {
                idChain = "#" + rootId;
            }
            if (mixedChain.Length > 0) {
                mixedChain = "#" + rootId + "/" + mixedChain;
            } else {
                mixedChain = "#" + rootId;
            }
        }
    ...
    string _BuildMlTargetSnippet(
        const string &in rootId,
        const string &in idChain,
        const string &in mixedChain,
        int layerIx,
        const string &in layerName
    ) {
    ...
    string _BuildMlFullSelectorPath(
        CGameUILayer@ layer,
        const string &in rootId,
        const string &in idChain,
        const string &in mixedChain
    ...
    void _MlStyleSnapshotInto(
        CGameManialinkControl@ n,
        Json::Value@ outObj,
        bool includeChildren,
        int maxDepth,
        int depth,
        bool includeTextValues
    ) {
    ...
    Json::Value@ _BuildMlStyleSnapshot(
        CGameManialinkControl@ n,
        bool includeChildren,
        int maxDepth,
        bool includeTextValues
    ...
    string _BuildControlTreeMixedPathForSelection() {
        ...
        CScene2d@ scene;
        if (!UiNavKit::Runtime::_GetScene2d(g_SelectedControlTreeOverlayAtSel, scene) || scene is null) return g_SelectedControlTreePath;
        uint rootIx = uint(g_SelectedControlTreeRootIx);
        if (rootIx >= scene.Mobils.Length) return g_SelectedControlTreePath;

        CControlFrame@ root = UiNavKit::Runtime::_RootFromMobil(scene, rootIx);
        ...
        if (relPath.Length > 0) {
            ...
            for (uint i = 0; i < parts.Length; ++i) {
                ...
                uint uidx = uint(idx);
                if (cur is null || uidx >= UiNavKit::Runtime::_ChildrenLen(cur)) {
                    tokens.InsertLast(part);
                    break;
                }

                auto ch = UiNavKit::Runtime::_ChildAt(cur, uidx);
                if (ch is null) {
                    tokens.InsertLast(part);
                    break;
                }
                ...
                if (idName.Length > 0) {
                    tokens.InsertLast("#" + idName);
                } else {
                    tokens.InsertLast("" + idx);
                }
                @cur = ch;
            }
        }
    ...
    bool _FindControlTreePathRecBudget(
        CControlBase@ cur,
        CControlBase@ target,
        const string &in relPath,
        string &out foundRelPath,
        ...
        for (uint i = 0; i < len; ++i) {
            auto ch = UiNavKit::Runtime::_ChildAt(cur, i);
            if (ch is null) continue;
    ...
    bool _FindControlTreePathForControlAtOverlayBudget(
        CControlBase@ target,
        uint overlay,
        int &out rootIx,
        string &out relPath,
    ...
    bool _FindControlTreePathForControlAtOverlay(
        CControlBase@ target,
        uint overlay,
        int &out rootIx,
        string &out relPath
    ...
    bool _FindControlTreePathForControlAnyOverlay(
        CControlBase@ target,
        uint &out overlay,
        int &out rootIx,
        string &out relPath
