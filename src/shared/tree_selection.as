namespace UiNavKit {
    ...
    bool _MlGetActiveNotesTooltip(
        const string &in layerKey,
        const string &in anchor,
        CGameManialinkControl@ n,
        CGameManialinkFrame@ layerRoot,
        string &out tooltip,
        int &out count
    ) {
    ...
    bool _TryParseMlUiPathLike(
        ...
        if (i >= parts.Length) {
            err = "Missing layer segment (expected L<ix>)";
            return false;
        }
        ...
        if (layerTok.Length > 0 && layerTok.StartsWith("L")) {
            ...
            if (layerIx < 0) {
                err = "Invalid layer index: " + layerTok;
                return false;
            }
            i++;
        } else {
    ...
    bool _TryBuildMlIndexPathFromNode(
        CGameManialinkFrame@ root,
        CGameManialinkControl@ node,
        string &out outPath,
        string &out err
    ) {
        outPath = "";
        ...
        while (cur !is null && !(cur is root) && guard < 512) {
            ...
            if (found < 0) {
                err = "Could not locate node index in parent";
                return false;
            }
    ...
    bool _TryJumpToMlSelectorAnyLayer(const string &in raw, int defaultAppKind, string &out status) {
        ...
        while (s.EndsWith("/")) s = s.SubStr(0, s.Length - 1);
        if (s.Length == 0) {
            status = "Invalid ML selector: empty";
            return false;
        }
        ...
        for (uint li = 0; li < layersLen; ++li) {
            ...
            UiNavKit::Inspectors::ManiaLink::g_MlUnifiedSourceSelectPending = appKind;
    ...
    bool _TryJumpToMlUiPathLike(const string &in raw, string &out status) {
        ...
        ("Jumped via selector: " + selector + " -> " + uiPath) : ("Jumped to path: " + uiPath);

        status = g_MlNodeFocusStatus;
        return true;
    }

    bool _TryParseControlTreeUiPathLike(
        const string &in raw,
        uint defaultOverlay,
        int defaultRootIx,
        uint &out overlay,
        ...
        if (s.StartsWith("O")) {
            ...
            if (j <= 1) {
                err = "Invalid overlay prefix";
                return false;
            }
            int ov = Text::ParseInt(s.SubStr(1, j - 1));
            if (ov < 0) {
                err = "Invalid overlay index";
                return false;
            }
            overlay = uint(ov);
            s = s.SubStr(j).Trim();
        ...
        if (s.StartsWith("root[")) {
            int end = s.IndexOf("]");
            if (end < 0) {
                err = "Invalid root[...] segment";
                return false;
            }
            rootIx = Text::ParseInt(s.SubStr(5, end - 5));
            if (rootIx < 0) {
                err = "Invalid root index";
                return false;
            }
            s = s.SubStr(end + 1).Trim();
            if (s.StartsWith("/")) s = s.SubStr(1);
    ...
    bool _TryJumpToControlTreeSelectorAnyRoot(const string &in raw, string &out status) {
        ...
        if (selector.Length == 0) {
            status = "Invalid ControlTree selector: empty";
            return false;
        }
        ...
        if (g_ControlTreeOverlay >= 0) {
            if (uint(g_ControlTreeOverlay) >= endOverlay) {
                status = "Overlay index out of range";
                return false;
            }
        ...
        for (uint ov = startOverlay; ov < endOverlay; ++ov) {
            auto found = UiNav::ResolvePathAnyRoot(selector, ov, 64);
            if (found is null) continue;
            ...
            UiNavKit::Inspectors::ControlTree::g_ControlTreeSelectionStatus = "Jumped to selector: " + selector + " -> " + uiPath;
            status = UiNavKit::Inspectors::ControlTree::g_ControlTreeSelectionStatus;
            return true;
        }
    ...
    bool _TryJumpToControlTreeUiPathLike(const string &in raw, string &out status) {
        ...
        if (g_ControlTreeOverlay >= 0) {
            defaultOverlay = uint(g_ControlTreeOverlay);
        } else if (g_SelectedControlTreeUiPath.Length > 0) {
            defaultOverlay = g_SelectedControlTreeOverlayAtSel;
        }

        int defaultRootIx = -1;
        if (g_SelectedControlTreeRootIx >= 0) {
            defaultRootIx = g_SelectedControlTreeRootIx;
        } else if (g_ControlTreeNodeFocusRootIx >= 0) {
            defaultRootIx = g_ControlTreeNodeFocusRootIx;
        }
        ...
        CScene2d@ scene;
        if (!UiNavKit::Runtime::_GetScene2d(overlay, scene) || scene is null) {
        ...
        UiNavKit::Inspectors::ControlTree::_ControlTreeExpandToUiPath(uiPath);
        UiNavKit::Inspectors::ControlTree::g_ControlTreeSelectionStatus = (selector.Length > 0 && selector != relPath) ?
        ("Jumped via selector: " + selector + " -> " + uiPath) : ("Jumped to path: " + uiPath);
    ...
    CGameManialinkControl@ _ResolveSelectedMlNode(string &out err) {
        ...
        for (uint i = 0; i < parts.Length; ++i) {
            ...
            int idx = Text::ParseInt(part);
            if (idx < 0) {
                err = "Invalid path segment: " + part;
                return null;
            }

            auto f = cast<CGameManialinkFrame@>(cur);
            if (f is null) {
                err = "Path points into non-frame";
                return null;
            }
            if (uint(idx) >= f.Controls.Length) {
                err = "Path index out of range: " + part;
                return null;
            }

            @cur = f.Controls[uint(idx)];
            if (cur is null) {
                err = "Null child at index: " + part;
                return null;
            }
        }
        return cur;
    ...
    CControlBase@ _ResolveSelectedControlTreeNode(string &out err) {
        ...
        if (g_SelectedControlTreeRootIx >= 0) {
            CScene2d@ scene;
            if (!UiNavKit::Runtime::_GetScene2d(overlay, scene)) {
                err = "No scene for overlay " + overlay;
                return null;
            }
            if (uint(g_SelectedControlTreeRootIx) >= scene.Mobils.Length) {
                err = "Root index out of range";
                return null;
            }
            CControlFrame@ root = UiNavKit::Runtime::_RootFromMobil(scene, uint(g_SelectedControlTreeRootIx));
            if (root is null) {
                err = "Root is null";
                return null;
            }
            ...
            for (uint i = 0; i < parts.Length; ++i) {
                ...
                uint len = UiNavKit::Runtime::_ChildrenLen(cur);
                if (uint(idx) >= len) {
                    err = "Path index out of range: " + part;
                    return null;
                }
                CControlBase@ ch = UiNavKit::Runtime::_ChildAt(cur, uint(idx));
                if (ch is null) {
                    err = "Null child at index: " + part;
                    return null;
                }
        ...
        if (g_SelectedControlTreePath.Length > 0) {
            auto found = UiNav::ResolvePathAnyRoot(g_SelectedControlTreePath, overlay, 64);
    ...
    bool _ResolveMlNodeByPath(
        int appKind,
        int layerIx,
        const string &in path,
        CGameManialinkFrame@&out root,
    ...
    bool _ResolveControlTreeNodeByPath(uint overlay, int rootIx, const string &in relPath, CControlBase@&out node) {
        ...
        CScene2d@ scene;
        if (!UiNavKit::Runtime::_GetScene2d(overlay, scene) || scene is null) return false;
        if (uint(rootIx) >= scene.Mobils.Length) return false;

        CControlFrame@ root = UiNavKit::Runtime::_RootFromMobil(scene, uint(rootIx));
        if (root is null) return false;
        ...
        for (uint i = 0; i < parts.Length; ++i) {
            ...
            int idx = Text::ParseInt(part);
            if (idx < 0) return false;
            uint len = UiNavKit::Runtime::_ChildrenLen(node);
            if (uint(idx) >= len) return false;
            @node = UiNavKit::Runtime::_ChildAt(node, uint(idx));
            if (node is null) return false;
        }
        return true;
    }
    ...
    void _DrawStackedTreeActionButtons(
        const string &in idBase,
        bool &out selectPressed,
        bool &out nodPressed,
        bool &out nodParentPressed
    ) {
        selectPressed = false;
        nodPressed = false;
    ...
    _ControlTreeNodeDataEntry@ _ControlTreeNodeData(
        CControlBase@ n,
        const string &in uiPath,
        bool needText = false,
        bool needVisible = false,
        ...
        if (!valid || e is null) {
            ...
            e.epoch = epoch;
            e.stampMs = now;
            e.type = UiNavKit::Runtime::NodeTypeName(n);
            e.label = _ColorizeTypeName(e.type);
            ...
            if (S_DebugTreeInlineText || needText) {
                e.text = UiNav::CleanUiFormatting(UiNavKit::Runtime::ReadText(n));
                e.hasText = true;
                if (S_DebugTreeInlineText && e.text.Length > 0) {
                    e.label += " | \"" + _TrimTreeText(e.text) + "\"";
                }
            ...
            if (needVisible) {
                e.visible = UiNavKit::Runtime::IsEffectivelyVisible(n);
                e.hasVisible = true;
            }
