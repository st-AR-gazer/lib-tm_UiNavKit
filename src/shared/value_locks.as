namespace UiNavKit {
    ...
    void _MlValueLocksSave() {
        ...
        for (uint i = 0; i < g_MlValueLocks.Length; ++i) {
            ...
            lines.InsertLast(_MlNoteEsc(id) + "\t" + (lock.enabled ? "1" : "0") + "\t" + lock.appKind + "\t" + lock.layerIx + "\t" + _MlNoteEsc(lock.path) + "\t" + lock.valueKind + "\t" + _MlNoteEsc(lock.lockedValue) + "\t" + _MlNoteEsc(lock.label));
        }
    ...
    bool _MlAddOrUpdateValueLockKind(
        int appKind,
        int layerIx,
        const string &in path,
        int kind,
        const string &in lockValue,
    ...
    bool _MlAddOrUpdateValueLock(
        int appKind,
        int layerIx,
        const string &in path,
        CGameManialinkControl@ selectedNode,
        const string &in lockValue
    ) {
